...
class PoseEstimation(object):
    def __init__(self):
        ...
        self.__box_model = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=True)
        self.__box_model.to(CTX)
        self.__box_model.eval()
        self.__pose_model = eval('models.' + cfg.MODEL.NAME + '.get_pose_net')(
            cfg, is_train=False
        )

        if cfg.TEST.MODEL_FILE:
            print('=> loading model from {}'.format(cfg.TEST.MODEL_FILE))
            self.__pose_model.load_state_dict(torch.load(cfg.TEST.MODEL_FILE), strict=False)
        else:
    ...
    def callback(self, msg):
        ...
        now = time.time()
        pred_boxes = get_person_detection_boxes(self.__box_model, image_per, threshold=0.9)
        then = time.time()
        rospy.loginfo("Find person bbox in: {} sec".format(then - now))
        ...
        now = time.time()
        pose_preds = get_pose_estimation_prediction(self.__pose_model, image_pose, centers, scales, transform=self.pose_transform)
        then = time.time()
        rospy.loginfo("Find person pose in: {} sec".format(then - now))
