...
RUN mkdir ros &&\
    ...
    cd ../../ &&\
    git clone --recursive https://github.com/Tacha-S/hrnet_pose.git src/hrnet_pose && \
    pip3 install torch torchvision numpy==1.19.5 && \
    pip3 install -r src/hrnet_pose/deep-high-resolution-net/requirements.txt && \
    cd src/hrnet_pose/deep-high-resolution-net/lib && \
    make &&\
    cd ../../../../.. && \
